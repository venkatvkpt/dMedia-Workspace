...
public class BusinessNewsController {
	...
	@RequestMapping(value = "/addBusinessNews", method = RequestMethod.POST)
		...
		if (dao.insertValuesNews(sql,bean)) {
			...
			redirectAttrs.addFlashAttribute("messenger", MessageDispatcher
					.message(AuditLog.userActivity(
							" Business News Article Submitted.", req), true));
			...
			redirectAttrs
					.addFlashAttribute(
							...
							MessageDispatcher
									.message(
											" Business News Article Submission is Failed,Try Again.",
											false));
		}

		return "redirect:/business.htm";
	}
