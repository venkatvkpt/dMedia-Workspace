...
@Controller
public class EducationAndEmployeementNewsController {

	@RequestMapping(value = "/eduemp", method = RequestMethod.GET)
	public ModelAndView latestNews(Model model) {
		...
		return new ModelAndView("eduemp");
	}

	@RequestMapping(value = "/addEduempNews", method = RequestMethod.POST)
	public String addLatestNews(Map<String, Object> map,
		...
		String table = "NEWS_EDU_EMP_EN";

		if (bean.getLanguage().equalsIgnoreCase("te")) {
			table = "NEWS_EDU_EMP_TE";
		} else if (bean.getLanguage().equalsIgnoreCase("hi")) {
			table = "NEWS_EDU_EMP_HI";
		} else if (bean.getLanguage().equalsIgnoreCase("ur")) {
			table = "NEWS_EDU_EMP_UR";
		} else {
			table = "NEWS_EDU_EMP_EN";
		}
		bean.setNews_id(dao.getNewsSeqValue(table));
		bean.setImagepath(ImageUpload.uploadImage(bean.getImage(), "een"));
		...
		if (dao.insertValuesNews(sql,bean)) {
			if(bean.getNotification().equalsIgnoreCase("true")){
				...
				NotificationSender.make(bean, title, "EEN");
			}

			redirectAttrs.addFlashAttribute("messenger", MessageDispatcher
					.message(AuditLog.userActivity(
							" Education and Employment News Article Submitted.", req), true));
			...
			redirectAttrs
					.addFlashAttribute(
							...
							MessageDispatcher
									.message(
											" Education and Employment News Article Submission is Failed,Try Again.",
											false));
		}

		return "redirect:/eduemp.htm";
	}
