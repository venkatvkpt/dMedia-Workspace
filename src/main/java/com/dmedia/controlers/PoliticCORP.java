...
public class PoliticCORP {
	...
	public ModelAndView execute(Model model) {
		...
		return new ModelAndView("corporation");
	}
@RequestMapping(value = "/addCORP", method = RequestMethod.POST)
	public String addLatestNews(Map<String, Object> map,
		...
		String table= "POLITICS_CORPORATION";
		boolean checkEn,checkTe,checkHi,checkUr;
		bean.setImgPath(ImageUpload.uploadImage(bean.getImage(), "pt\\corp"));
		...
		if(checkEn && checkTe && checkHi && checkUr){
			redirectAttrs.addFlashAttribute("messenger", MessageDispatcher
					.message(AuditLog.userActivity(
							"Corporation Information Submitted in Different Languages.", req), true));
		}else{
			dao.deletePoliticsByImagePath(table, bean.getImgPath());
			redirectAttrs.addFlashAttribute("messenger", MessageDispatcher
					.message(AuditLog.userActivity(
							"Corporation Information Submission is Failed,Try Again.", req), false));		
		}
		
		return "redirect:/corporation.htm";
