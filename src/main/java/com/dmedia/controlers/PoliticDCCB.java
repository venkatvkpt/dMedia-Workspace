...
@Controller
public class PoliticDCCB {
	@RequestMapping(value = "/dccb", method = RequestMethod.GET)
	public ModelAndView execute(Model model) {
		PoliticiansBean bean = new PoliticiansBean();
		model.addAttribute("politics", bean);
		return new ModelAndView("dccb");
	}
@RequestMapping(value = "/addDCCB", method = RequestMethod.POST)
	public String addLatestNews(Map<String, Object> map,
		...
		String table= "POLITICS_DCCB";
		boolean checkEn,checkTe,checkHi,checkUr;
		bean.setImgPath(ImageUpload.uploadImage(bean.getImage(), "pt\\dccb"));
		...
		if(checkEn && checkTe && checkHi && checkUr){
			redirectAttrs.addFlashAttribute("messenger", MessageDispatcher
					.message(AuditLog.userActivity(
							"DCCB Information Submitted in Different Languages.", req), true));
		}else{
			dao.deletePoliticsByImagePath(table, bean.getImgPath());
			redirectAttrs.addFlashAttribute("messenger", MessageDispatcher
					.message(AuditLog.userActivity(
							"DCCB Information Submission is Failed,Try Again.", req), false));		
		}
		
		return "redirect:/dccb.htm";
