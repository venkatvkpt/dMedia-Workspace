...
@Controller
public class PoliticMLA {
	@RequestMapping(value = "/mla", method = RequestMethod.GET)
	public ModelAndView execute(Model model) {
		PoliticiansBean bean = new PoliticiansBean();
		model.addAttribute("politics", bean);
		return new ModelAndView("mla");
	}
	
	@RequestMapping(value = "/addMLA", method = RequestMethod.POST)
	public String addLatestNews(Map<String, Object> map,
		...
		String table= "POLITICS_MLA";
		boolean checkEn,checkTe,checkHi,checkUr;
		bean.setImgPath(ImageUpload.uploadImage(bean.getImage(), "pt\\mla"));
		...
		if(checkEn && checkTe && checkHi && checkUr){
			redirectAttrs.addFlashAttribute("messenger", MessageDispatcher
					.message(AuditLog.userActivity(
							"MLA Information Submitted in Different Languages.", req), true));
		}else{
			dao.deletePoliticsByImagePath(table, bean.getImgPath());
			redirectAttrs.addFlashAttribute("messenger", MessageDispatcher
					.message(AuditLog.userActivity(
							"MLA Information Submission is Failed,Try Again.", req), false));		
		}
		
		return "redirect:/mla.htm";
