...
@RequestMapping(value = "/addMP", method = RequestMethod.POST)
	public String addLatestNews(Map<String, Object> map,
		...
		String table= "POLITICS_MP";
		boolean checkEn,checkTe,checkHi,checkUr;
		bean.setImgPath(ImageUpload.uploadImage(bean.getImage(), "pt\\mp"));
		...
		if(checkEn && checkTe && checkHi && checkUr){
			redirectAttrs.addFlashAttribute("messenger", MessageDispatcher
					.message(AuditLog.userActivity(
							"MP Information Submitted in Different Languages.", req), true));
		}else{
			dao.deletePoliticsByImagePath(table, bean.getImgPath());
			redirectAttrs.addFlashAttribute("messenger", MessageDispatcher
					.message(AuditLog.userActivity(
							"MP Information Submission is Failed,Try Again.", req), false));		
		}
		
		return "redirect:/mp.htm";
