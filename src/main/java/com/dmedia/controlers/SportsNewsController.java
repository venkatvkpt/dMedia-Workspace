...
public class SportsNewsController {
	...
	public ModelAndView latestNews(Model model) {
		...
		return new ModelAndView("sports");
	}
		...
		if (dao.insertValuesNews(sql,bean)) {
			...
			redirectAttrs
					.addFlashAttribute(
							...
							MessageDispatcher
									.message(
											" Sports News Article Submission is Failed,Try Again.",
											false));
		}

		return "redirect:/sports.htm";
	}
