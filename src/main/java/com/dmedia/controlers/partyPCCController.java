...
@Controller
public class partyPCCController {
...
@RequestMapping(value = "/addPCC", method = RequestMethod.POST)
	public String addLatestNews(Map<String, Object> map,
		...
		String table= "PARTY_PCC";
		boolean checkEn,checkTe,checkHi,checkUr;
		bean.setImgPath(ImageUpload.uploadImage(bean.getImage(), "pc\\pcc"));
		...
		if(checkEn && checkTe && checkHi && checkUr){
			redirectAttrs.addFlashAttribute("messenger", MessageDispatcher
					.message(AuditLog.userActivity(
							"PCC Committees Information Submitted in Different Languages.", req), true));
		}else{
			dao.deletePoliticsByImagePath(table, bean.getImgPath());
			redirectAttrs.addFlashAttribute("messenger", MessageDispatcher
					.message(AuditLog.userActivity(
							"PCC Committees Information Submission is Failed,Try Again.", req), false));		
		}
		
		return "redirect:/pcc.htm";
